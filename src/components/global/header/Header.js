...
const megaMenuProducts = [
  {
    ...
    items: ["Plain Khakhra", "Jeera Khakhra", "Methi Khakhra", "Masala Khakhra", "Diet Khakhra"],
  },
  {
    category: "Spicy & Tangy",
    items: ["Peri Peri Khakhra", "Pudina Khakhra", "Garlic Khakhra", "Chaat Masala", "Panipuri Flavor"],
  },
  {
    category: "Premium Fusion",
    items: ["Cheese Khakhra", "Pizza Khakhra", "Manchurian", "Chorafali Khakhra", "Mathiya Special"],
  },
];
...
const Header = () => {
  ...
  return (
    <header className="w-full relative z-100">
      ...
      <div className={`bg-[#facc15] transition-all duration-300 overflow-hidden ${isScrolled ? "h-0 opacity-0" : "h-auto py-2 opacity-100"}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full flex flex-col md:flex-row justify-between items-center text-xs md:text-sm text-black">
          <div className="flex flex-wrap justify-center md:justify-start items-center space-x-4 mb-2 md:mb-0">
            ...
            <a href="mailto:info.cruncheskhakhra@gmail.com" className="hidden md:flex hover:text-white items-center transition cursor-pointer font-medium">
          ...
          <div className="items-center md:block hidden justify-center space-x-4 font-medium">
            ...
            <Link href="/store" className="hover:text-white transition">Store Locator</Link>
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
          <div className={`flex justify-between items-center transition-all duration-300 ${isScrolled ? "h-20" : "h-20"}`}>
            {/* Logo */}
            <Link href="/" className="shrink-0 flex items-center transform hover:scale-105 transition duration-300">
              <Image
            ...
            <div className="hidden lg:flex space-x-8 items-center h-full">
              {navLinks.map((link) => {
                ...
                return (
                  <div key={link.name} className="h-full flex items-center group">
                    <Link
                      href={link.path}
                        ...
                        <div className="max-w-7xl mx-auto px-8 py-8 flex gap-8">
                          <div className="flex-1 grid grid-cols-3 gap-6">
                            {megaMenuProducts.map((group, idx) => (
                              <div key={idx}>
                                <h3 className="text-sm font-bold text-gray-900 mb-4 uppercase tracking-wider">{group.category}</h3>
                          ...
                          <div className="w-87.5 grid grid-cols-2 gap-4">
                            <div className="relative group/card cursor-pointer rounded-lg overflow-hidden h-48 bg-gray-100">
                              <div className="absolute inset-0 bg-yellow-100 flex items-center justify-center p-4 transition-transform duration-500 group-hover/card:scale-110">
                                <span className="text-yellow-800 font-bold text-center">Trending<br />Masala Khakhra</span>
                              </div>
                              <div className="absolute inset-0 bg-black/20 opacity-0 group-hover/card:opacity-100 transition-opacity" />
                            </div>
                            <div className="relative group/card cursor-pointer rounded-lg overflow-hidden h-48 bg-gray-100">
                              <div className="absolute inset-0 bg-green-100 flex items-center justify-center p-4 transition-transform duration-500 group-hover/card:scale-110">
                                <span className="text-green-800 font-bold text-center">New Arrival<br />Pizza Khakhra</span>
                              </div>
                              <div className="absolute inset-0 bg-black/20 opacity-0 group-hover/card:opacity-100 transition-opacity" />
                            </div>
                          </div>
            ...
            <div className="flex items-center space-x-4 md:space-x-6 text-gray-700">
              <button className="hover:text-yellow-500 transition transform hover:scale-110 hidden md:block"><Search className="w-5 h-5" /></button>
              <button className="hover:text-yellow-500 transition transform hover:scale-110 hidden md:block"><User className="w-5 h-5" /></button>
              <button className="hover:text-yellow-500 transition transform hover:scale-110 relative">
                <ShoppingCart className="w-5 h-5" />
                <span className="absolute -top-2 -right-2 bg-yellow-500 text-white text-[10px] font-bold rounded-full h-4 w-4 flex items-center justify-center">2</span>
              </button>
              <button className="lg:hidden text-gray-700 hover:text-yellow-500 transition" onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}>
        ...
        {/* --- MOBILE MENU --- */}
        <div className={`lg:hidden absolute top-full left-0 w-full bg-white shadow-lg overflow-hidden transition-all duration-300 ease-in-out border-t ${isMobileMenuOpen ? "max-h-screen opacity-100 py-4" : "max-h-0 opacity-0 py-0 border-transparent"}`}>
          <div className="px-4 space-y-2">
            {navLinks.map((link) => {
              const isActive = pathname === link.path;
              return (
                <div key={link.name}>
                  <Link href={link.path} className={`block py-3 px-4 rounded-md font-medium transition ${isActive ? "bg-yellow-50 text-yellow-600" : "text-gray-700 hover:bg-gray-50 hover:text-yellow-500"}`} onClick={() => setIsMobileMenuOpen(false)}>
                    {link.name}
                  </Link>
                  {link.hasMegaMenu && (
                    <div className="pl-8 pr-4 py-2 grid grid-cols-2 gap-2">
                      {megaMenuProducts[0].items.slice(0, 4).map((item, i) => (
                        <Link key={i} href="#" className="text-sm text-gray-500 hover:text-yellow-500 py-1 block">{item}</Link>
                      ))}
                      <Link href="/products" className="text-sm font-semibold text-yellow-600 py-1 block">View All &rarr;</Link>
                    </div>
                  )}
                </div>
              );
